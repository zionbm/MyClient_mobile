...
class RecentActionsScreen extends StatefulWidget {
  const RecentActionsScreen({super.key, required this.controller});
  final SessionController controller;

  @override
  State<RecentActionsScreen> createState() => _RecentActionsScreenState();
}

class _RecentActionsScreenState extends State<RecentActionsScreen> {
        ...
        Expanded(
          child: FutureBuilder<Map<String, Object?>>(
            ...
            builder: (context, snapshot) {
              ...
              if (batches.isEmpty) {
                return const Center(child: Text('עדיין אין פעולות '));
              }
  ...
  Future<void> _load() async {
    ...
    setState(() {
      _future = widget.controller.apiClient.actionBatches.list(
  ...
  Future<void> _loadPreferences() async {
    final session = widget.controller.session!;
    final json = await widget.controller.apiClient.actionBatches.preferences(
      firebaseUid: session.firebaseUid,
      mockPhoneNumber: session.mockPhoneNumber,
    );
    if (!mounted) return;
