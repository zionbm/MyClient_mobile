...
class _TasksScreenState extends State<TasksScreen> {
  ...
  late final PagingController<Task> _paging;
  _TaskView _view = _TaskView.open;

  @override
  void initState() {
    super.initState();
    _paging = PagingController<Task>(_loadPage, itemKey: (task) => task.id);
    _load();
  }
  ...
  Widget _openTasks(List<Task> allTasks) {
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    final tomorrow = today.add(const Duration(days: 1));
    final open =
        allTasks.where((task) => task.status == TaskStatus.open).toList()
          ..sort(_compareTasks);
  ...
  Widget _closedTasks(List<Task> tasks) {
  ...
  Widget _taskCard(Task task) => _TaskCard(
    task: task,
    onEdit: () => _edit(task),
  ...
  Future<pagination.Page<Task>> _loadPage(String? cursor) {
    final session = widget.controller.session!;
    return widget.controller.apiClient.tasks.list(
  ...
  Future<void> _complete(Task task) => _runTaskAction(task, () {
    final session = widget.controller.session!;
    return widget.controller.apiClient.tasks.lifecycle(
      businessId: session.businessId!,
      taskId: task.id,
  ...
  Future<void> _postpone(Task task) => _runTaskAction(task, () {
    final session = widget.controller.session!;
    ...
    return widget.controller.apiClient.tasks.update(
      businessId: session.businessId!,
      taskId: task.id,
      firebaseUid: session.firebaseUid,
      mockPhoneNumber: session.mockPhoneNumber,
  ...
  static int _compareTasks(Task left, Task right) {
  ...
  static int _compareClosedTasks(Task left, Task right) {
...
class _TaskSection extends StatelessWidget {
  ...
  final Widget Function(Task task) builder;
              ...
              children: [
                Icon(
                  task.status == TaskStatus.open
                      ...
                      : Icons.task_alt,
                  color: task.status == TaskStatus.open
                      ? AppColors.primary
                      : AppColors.success,
                ),
