...
class PendingActionsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: PendingActionsPanel(controller: controller),
  );
}

class PendingActionsPanel extends StatefulWidget {
  ...
  @override
  State<PendingActionsPanel> createState() => _PendingActionsPanelState();
}

class _PendingActionsPanelState extends State<PendingActionsPanel> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      _future = widget.controller.apiClient.assistant.listPending(
  ...
  Future<void> _resolve(
    ...
    try {
      final result = await widget.controller.apiClient.assistant.resolvePending(
        businessId: session.businessId!,
  ...
  Future<void> _reject(Map<String, Object?> action) async {
    ...
    try {
      await widget.controller.apiClient.assistant.rejectPending(
        businessId: session.businessId!,
        pendingActionId: actionId,
        firebaseUid: session.firebaseUid,
        mockPhoneNumber: session.mockPhoneNumber,
...
class _PendingPayloadFormState extends State<_PendingPayloadForm> {
  ...
  Future<Map<String, List<PendingEntityChoice>>> _loadChoices(
    ...
    if (fields.contains('taskId')) {
      final page = await widget.controller.apiClient.tasks.list(
        businessId: session.businessId!,
        firebaseUid: session.firebaseUid,
        mockPhoneNumber: session.mockPhoneNumber,
      );
    ...
    if (fields.contains('entityId')) {
      final pages = await Future.wait([
        widget.controller.apiClient.activities.list(
