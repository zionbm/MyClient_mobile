...
import 'home/today_overview.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<TodayOverview>? _future;
  late final TodayOverviewLoader _overviewLoader;
  ...
  void initState() {
    super.initState();
    _overviewLoader = TodayOverviewLoader(widget.controller.apiClient);
        ...
        child: ListView(
          ...
          children: [
            ...
            if (snapshot.connectionState == ConnectionState.waiting)
              ...
              _buildToday(snapshot.data ?? const TodayOverview()),
          ],
        ),
      ),
    );
  ...
  Widget _buildToday(TodayOverview data) {
    final dueTasks = [
      ...data.overdueTasks,
      ...data.todayTasks,
      ...data.undatedTasks,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _QuickCreateBar(
            onTask: _createTask,
            onJob: () => _create(ActivityKind.job),
            onVisit: () => _create(ActivityKind.visit),
          ),
        ],
      ),
    );
  ...
  Future<void> _openTasks() async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => TasksScreen(controller: widget.controller),
      ),
    );
    await _load();
  }
  ...
  Future<void> _lifecycle(Activity item, ActivityAction action) async {
    final session = widget.controller.session!;
    ...
    try {
      final updated = await widget.controller.apiClient.activities.lifecycle(
        kind: item.kind,
        businessId: session.businessId!,
        entityId: item.id,
        action: action,
      ...
      if (!mounted) return;
      final message =
          action == ActivityAction.reportCompleted &&
              updated.status == ActivityStatus.open
  ...
  Future<bool> _openAmount(Activity item) async {
    await showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
      builder: (_) =>
          AmountSheet(controller: widget.controller, activity: item),
    );
    widget.controller.markDataChanged({DataScope.crm});
    await _load();
    final session = widget.controller.session!;
