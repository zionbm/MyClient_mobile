...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Widget _activityCard(Activity activity) => Padding(
  ...
  Future<_CalendarData> _loadCalendarData(
    ...
    final scheduledFuture = widget.controller.apiClient.activities.schedule(
    ...
    final tasksFuture = widget.controller.apiClient.tasks.listAll(
      businessId: session.businessId!,
      firebaseUid: session.firebaseUid,
      mockPhoneNumber: session.mockPhoneNumber,
      state: 'OPEN',
    ...
    final completedFuture = widget.controller.apiClient.activities.completed(
      businessId: session.businessId!,
      firebaseUid: session.firebaseUid,
      mockPhoneNumber: session.mockPhoneNumber,
      from: range.$1,
    ...
    late List<Activity> scheduledItems;
    late List<Activity> jobs;
    late List<Activity> visits;
    late List<Task> taskItems;
    late List<CompletedItem> completed;
  ...
  Future<void> _showCreateMenu() async {
    ...
    if (type == _CalendarCreateType.job) await _create(ActivityKind.job);
    if (type == _CalendarCreateType.visit) await _create(ActivityKind.visit);
  }
  ...
  Future<void> _createTask() async {
    final created = await showModalBottomSheet<Task>(
      ...
      useSafeArea: true,
      builder: (_) => TaskForm(controller: widget.controller),
    );
  ...
  Future<void> _create(ActivityKind kind) async {
  ...
  Future<void> _lifecycle(Activity activity, ActivityAction action) async {
    final session = widget.controller.session!;
    var body = const <String, Object?>{};
    if (action == ActivityAction.reportCompleted) {
  ...
  Future<bool> _openAmount(Activity activity) async {
    ...
    final session = widget.controller.session!;
    try {
      await widget.controller.apiClient.amounts.get(
        kind: activity.kind,
        businessId: session.businessId!,
        entityId: activity.id,
        firebaseUid: session.firebaseUid,
  ...
  Future<void> _showAvailability() async {
    final session = widget.controller.session!;
    try {
      final result = await widget.controller.apiClient.activities.availability(
        businessId: session.businessId!,
        firebaseUid: session.firebaseUid,
        mockPhoneNumber: session.mockPhoneNumber,
        date: _selectedDate,
...
class _CalendarData {
  ...
  final List<Activity> scheduled;
  final List<Task> tasks;
  final List<Activity> unscheduled;
...
class _AgendaEntry {
  ...
  factory _AgendaEntry.activity(Activity activity) =>
      _AgendaEntry._(activity: activity, startsAt: activity.startsAt!);
  factory _AgendaEntry.task(Task task) =>
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: BorderDirectional(
            start: BorderSide(
              color: activity.kind == ActivityKind.job
                  ? AppColors.primary
                  : AppColors.visit,
              width: 4,
            ),
