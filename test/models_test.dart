...
void main() {
  test(' customer parses contact details, tasks and notes', () {
    ...
    expect(customer.tasks.single.status, TaskStatus.open);
    expect(customer.notes.single.status, NoteStatus.done);
    expect(customer.version, 3);
  });

  test(' task parses its actual completion time', () {
  ...
  test(' activities apply the product default display durations', () {
    ...
    final job = Activity.fromJson(base, ActivityKind.job);
    final visit = Activity.fromJson(base, ActivityKind.visit);
  ...
  test(' amount parses decimal strings and calculates the balance', () {
    final amount = Amount.fromJson(const {
      'id': 'amount-1',
      'totalAmount': '1500.00',
      'paidAmount': '500.00',
    ...
    expect(amount.status, PaymentStatus.partiallyPaid);
  });
}
