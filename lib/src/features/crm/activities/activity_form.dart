...
class _ActivityFormState extends State<ActivityForm> {
      ...
      children: [
        FutureBuilder<List<Customer>>(
          ...
          builder: (context, snapshot) => DropdownButtonFormField<String>(
            ...
            items: (snapshot.data ?? const <Customer>[])
                .map(
        ...
        FutureBuilder<List<Customer>>(
          ...
          builder: (context, snapshot) {
            ...
            final selected = customers
                ...
                .firstOrNull;
            final addresses = selected?.addresses ?? const <ServiceAddress>[];
  ...
  Future<void> _pickDateTime() async {
    final previousDuration = _startsAt != null && _endsAt != null
        ...
        : Duration(minutes: widget.kind == ActivityKind.job ? 120 : 60);
  ...
  Future<void> _save({String? scheduleConflictToken}) async {
    ...
    try {
      ...
      final activity = widget.activity == null
          ? await widget.controller.apiClient.activities.create(
              kind: widget.kind,
              businessId: session.businessId!,
              firebaseUid: session.firebaseUid,
              mockPhoneNumber: session.mockPhoneNumber,
          ...
          : await widget.controller.apiClient.activities.update(
              kind: widget.kind,
              businessId: session.businessId!,
